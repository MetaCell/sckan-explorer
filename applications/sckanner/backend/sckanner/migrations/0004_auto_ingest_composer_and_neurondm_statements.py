...
def create_neurondm_and_composer_data_sources(apps, schema_editor):
    ...
    neuron_source, _ = DataSource.objects.get_or_create(
        ...
        raw_json_data_for_statements_retrival=None,
        python_code_file_for_statements_retrival=None
    )
    script_path = os.path.join(os.path.dirname(__file__), 'data', 'neurondm_ingestion_script.py')
    with open(script_path, 'rb') as f:
        neuron_source.python_code_file_for_statements_retrival.save(
            'neurondm_ingestion_script.py',
            File(f),
            save=True
        )
    neuron_source.raw_json_data_for_statements_retrival = None
    neuron_source.save()
    ...
    with open(script_path, 'rb') as f:
        composer_source.python_code_file_for_statements_retrival.save(
            'composer_ingestion_script.py',
            File(f),
            save=True
        )
    composer_source.raw_json_data_for_statements_retrival = None
