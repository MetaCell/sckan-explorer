...
const ReleaseDropdown = ({
	...
	selectedDatasnaphshot: string, setSelectedDatasnaphshot: (datasnaphshot: string) => void
}) => {
	...
	const getGroupNameForSelected = (groupedOptions: { group: string, options: Datasnapshot[] }[], selectedId: string) => {
			...
			<Box sx={{ position: 'relative', minWidth: 220 }}>
				...
				>
					...
					<Box
						...
						sx={styles.dropdownScrollbox}
					>
						{filteredGroups.map((group) =>
							group.options.length > 0 ? (
								<Box key={group.group} sx={{ borderBottom: `1px solid ${gray700}`, paddingX: '0.75rem', paddingBottom: '0.25rem' }}>
