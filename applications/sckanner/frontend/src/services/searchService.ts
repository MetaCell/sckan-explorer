...
export const searchNerveFilter = (
  value: string,
  nerveOptions: Option[],
): Option[] => {
...
export const searchPhenotypeFilter = (
  value: string,
...
const searchAnatomicalEntities = (
  searchValue: string,
  options: Option[],
): Option[] => {
  const lowerSearchValue = searchValue.toLowerCase();
  return options.filter((option) => {
    ...
    const synonymMatch = option.content.some(
      (detail) =>
        detail.title === SYNONYMS_TITLE &&
        detail.value.toLowerCase().includes(lowerSearchValue),
    );
