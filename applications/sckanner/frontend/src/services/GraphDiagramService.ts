...
export const processData = ({
  ...
  const afferentTerminalIds = [...entityMap.entries()]
    .filter(
      ([_, info]) =>
        info.nodeType === NodeTypes.Destination &&
  ...
  nonAfferentDestinationIds.forEach((entityId) => {
    const {
      ...
      links: traversalLinks,
      maxLevel,
    } = traverseFromNonAfferentTerminal(
      entityId,
      linkUUIDtoIds,
      entityMap,
      nodeMap,
...
function addDisconnectedNodes(
  ...
  const disconnectedEntities = [...entityMap.entries()].filter(
    ([id, _]) => !nodeMap.has(id),
  );
  ...
  const yPos =
    POSITION_CONSTANTS.yStart + startingLevel * POSITION_CONSTANTS.yIncrement;
...
function collectEntityMap(
  ...
  const getOrCreateEntityInfo = (
    entityId: string,
    entity: AnatomicalEntity,
    nodeType: NodeTypes = NodeTypes.Via,
  ): EntityInfo => {
  ...
  destinations?.forEach((destination) => {
    ...
    destination.anatomical_entities.forEach((entity) => {
      ...
      const entityId = entity.id.toString();
      mapUUIDtoId(uuid, entityId);
      const entityInfo = getOrCreateEntityInfo(
        uuid,
        entity,
  ...
  vias?.forEach((via) => {
    via.anatomical_entities.forEach((entity) => {
      const entityId = entity.id.toString();
      const entitiesInfo = linkUUIDtoIds
        ?.get(entityId)
        ?.filter((id) => {
      ...
      via.from_entities
        .map((fromEntity) => fromEntity.id.toString())
        .map((id) => linkUUIDtoIds?.get(id))
        .forEach((uuids) => {
          uuids?.forEach((uuid) => {
...
function traverseFromNonAfferentTerminal(
  ...
  const stack: { id: string; level: number }[] = [
    { id: entityId, level: initialLevel },
  ];
...
function adjustNodesYPosition(
  ...
  traversalNodes.forEach(({ node, level }) => {
    const defaultY =
      POSITION_CONSTANTS.yStart + level * POSITION_CONSTANTS.yIncrement;

    setPosition(
      node,
      node.getX(),
      defaultY,
      traversalNodes.map((n) => n.node),
    );
  });
}
...
function getEntityNameAndUri(entity: AnatomicalEntity): {
  name: string;
  ontology_uri?: string;
} {
...
function findNonOverlappingPosition(
  ...
  do {
    ...
    collision = nodes.some((node) => {
      return (
        Math.abs(node.getX() - currentX) < 50 &&
        Math.abs(node.getY() - currentY) < 50
      );
    });
