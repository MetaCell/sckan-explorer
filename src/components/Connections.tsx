...
import {HierarchicalItem, PhenotypeKsIdMap, SummaryType, KsMapType} from "./common/Types";
...
function Connections() {
    ...
    useEffect(() => {
        ...
        if (selectedConnectionSummary && selectedConnectionSummary.hierarchy && hierarchicalNodes) {
    ...
    }, [hierarchicalNodes, selectedConnectionSummary, summaryFilters, knowledgeStatements]);

    const selectedPhenotypes = useMemo(() => getAllPhenotypes(connectionsMap), [connectionsMap]);
                ...
                <>
                    <Box p={3} display='flex' flexDirection='column' gap={3}>
                        <Box display='flex' alignItems='flex-end' gap={1.5}>
                            <Box flex={1}>
                                <Typography sx={{...styles.heading, marginBottom: '0.75rem'}}>Connection
                                    origin</Typography>
                    ...
                    <Box display='flex' flexDirection='column' flex={1} p={3} sx={{
                        ...
                        <Box mb={3}>
                            <Typography sx={{...styles.heading, fontSize: '1rem', lineHeight: '1.5rem'}}>Summary
                                map</Typography>
                            <Typography sx={styles.text}>
                                Summary map shows the connections of the selected connection origin and end organ with
                                phenotypes. Select individual squares to view the details of each connections.
                            </Typography>
                        </Box>
                        <SummaryFiltersDropdown nerves={nerves} phenotypes={selectedPhenotypes}/>
                        <HeatmapGrid
                            yAxis={yAxis}
                            setYAxis={setYAxis}
                            xAxis={xAxis}
