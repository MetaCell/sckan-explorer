...
export const generatePhenotypeColors = (num: number) => {
    const scale = chroma.scale(['red', 'blue', 'green', 'yellow', 'purple', 'orange', 'brown']).mode('lch').colors(num);
    return scale;
};
...
export function getAllPhenotypes(connections: Map<string, PhenotypeKsIdMap[]>): string[] {
